...
import 'mod/app_settings_model.dart';
...
class DatabaseHelper {
  ...
  Future<void> _createAllTables(Database db, int version) async {
    ...
    ''');
    // في قسم إنشاء الجداول، استبدل جدول settings الحالي:
      await db.execute('''
    CREATE TABLE IF NOT EXISTS settings (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      setting_key TEXT UNIQUE NOT NULL,
      setting_value TEXT,
      type TEXT CHECK(type IN ('string', 'int', 'double', 'bool')) DEFAULT 'string',
  ...
  // إضافة جدول الوحدات (الذي كان ناقصاً)
      await db.execute('''
  ...
  ''');
    ...
    // 29. جدول سجل تغييرات الإعدادات (للتدقيق)
    await db.execute('''
...
// 30. جدول صلاحيات الإعدادات (للتحكم بالوصول)
    await db.execute('''
  CREATE TABLE settings_permissions (
    ...
    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(setting_key, user_role)
  )
''');

// 31. جدول قوالب الإعدادات (للاستيراد/التصدير)
    await db.execute('''
  CREATE TABLE settings_templates (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    template_name TEXT NOT NULL,
    template_description TEXT,
    settings_data TEXT NOT NULL, -- JSON format
    created_by INTEGER,
...
// 32. جدول الإعدادات الخاصة بالمخازن
    await db.execute('''
...
''');
  ...
  CREATE TABLE IF NOT EXISTS app_settings (
    ...
    setting_value TEXT,
   setting_type TEXT NOT NULL
      CHECK(setting_type IN ('string', 'int', 'double', 'bool', 'json')),
    category TEXT NOT NULL CHECK(category IN ('general', 'product', 'invoice', 'system', 'barcode', 'security', 'backup', 'advanced')),
    group_name TEXT,
...
// إضافة جدول متابعة النسخ الاحتياطي
    await db.execute('''
  CREATE TABLE IF NOT EXISTS backup_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    backup_type TEXT CHECK(backup_type IN ('manual', 'auto', 'system')) NOT NULL,
...
// إضافة جدول سجل الصيانة
    await db.execute('''
  CREATE TABLE IF NOT EXISTS maintenance_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    operation TEXT CHECK(operation IN ('compress', 'reindex', 'integrity_check', 'clear_data', 'reset')) NOT NULL,
    details TEXT,
    status TEXT CHECK(status IN ('success', 'failed')) NOT NULL,
    execution_time INTEGER, -- في الميلي ثانية
    executed_by INTEGER,
...
''');
    ...
    await db.execute('''
      ALTER TABLE products ADD COLUMN min_quantity INTEGER DEFAULT 10
    ''');

// إنشاء فهرس
      await db.execute('''
    ALTER TABLE app_settings ADD COLUMN validation_regex TEXT
  ''');

      await db.execute('''
    ALTER TABLE app_settings ADD COLUMN depends_on TEXT
  ''');

      await db.execute('''
    ALTER TABLE app_settings ADD COLUMN visible_condition TEXT
  ''');

      await db.execute('''
    ALTER TABLE app_settings ADD COLUMN tooltip TEXT
  ''');
  ...
  Future<void> _seedSettingsData(Database db) async {
    // إعدادات عامة
    final defaultSettings = [
      // الإعدادات العامة
      {
        ...
        'setting_type': 'string',
        'category': 'general',
        'description': 'اسم الشركة المعروض في الفواتير والتقارير',
        'default_value': 'شركة إدارة المخزون'
      },
      {
      ...
      },
      {
        ...
        'setting_type': 'bool',
        'category': 'general',
        'description': 'تفعيل التنبيهات والإشعارات',
        'default_value': '1'
      },

      // إعدادات الباركود
      {
        'setting_key': 'barcode_type',
        'setting_value': 'CODE128',
        'setting_type': 'string',
        'category': 'barcode',
        'options': '["CODE128", "CODE39", "EAN13", "EAN8", "UPC-A", "QR"]',
        'description': 'نوع الباركود المستخدم',
        'default_value': 'CODE128'
      },
      {
        'setting_key': 'barcode_width',
        'setting_value': '2',
        'setting_type': 'int',
        'category': 'barcode',
        'min_value': '1',
        'max_value': '10',
        'description': 'عرض الباركود (مم)',
        'default_value': '2'
      },
      {
        'setting_key': 'barcode_height',
        'setting_value': '100',
        'setting_type': 'int',
        'category': 'barcode',
        'min_value': '50',
        'max_value': '500',
        'description': 'طول الباركود (مم)',
        'default_value': '100'
      },
      {
        'setting_key': 'barcode_include_price',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'barcode',
        'description': 'عرض سعر المنتج على الباركود',
        'default_value': '0'
      },
      {
        'setting_key': 'barcode_include_name',
        'setting_value': '1',
        'setting_type': 'bool',
        'category': 'barcode',
        'description': 'عرض اسم المنتج على الباركود',
        'default_value': '1'
      },

      // إعدادات متقدمة
      {
        'setting_key': 'track_serial_numbers',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'تفعيل تتبع الأرقام التسلسلية للمنتجات',
        'default_value': '0'
      },
      {
        'setting_key': 'track_expiry_dates',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'تفعيل تتبع تواريخ انتهاء الصلاحية',
        'default_value': '0'
      },
      {
        'setting_key': 'auto_print_invoice',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'الطباعة التلقائية للفواتير',
        'default_value': '0'
      },
      {
        'setting_key': 'auto_purchase_orders',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'إنشاء طلبات شراء تلقائية عند انخفاض المخزون',
        'default_value': '0'
      },
      {
        'setting_key': 'email_reports',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'تفعيل إرسال التقارير عبر البريد الإلكتروني',
        'default_value': '0'
      },
      {
      ...
      {
        'setting_key': 'ip_restriction',
        'setting_value': '0',
        'setting_type': 'bool',
        'category': 'advanced',
        'description': 'تقييد الوصول بعناوين IP محددة',
        'default_value': '0'
      },
      {
        'setting_key': 'app_language',
        'setting_value': 'العربية',
        'setting_type': 'string',
        'category': 'advanced',
        'options': '["العربية", "English", "Français", "Español"]',
        'description': 'لغة التطبيق الافتراضية',
        'default_value': 'العربية'
      },
      {
        ...
        'setting_type': 'string',
        'category': 'advanced',
        'options': '["فاتح", "داكن", "تلقائي بالنظام"]',
        'description': 'مظهر التطبيق',
        'default_value': 'فاتح'
      },
    ];
    ...
    for (var setting in defaultSettings) {
      try {
        ...
        // تجاهل إذا كان المفتاح موجودًا بالفعل
      }
    }
  }
  ...
  Future<void> _createIndexes(Database db) async {
    ...
    await db.execute('CREATE INDEX idx_settings_key ON app_settings(setting_key)');
    await db.execute('CREATE INDEX idx_settings_category ON app_settings(category)');
    await db.execute('CREATE INDEX idx_settings_audit_setting ON settings_audit_log(setting_key)');
    await db.execute('CREATE INDEX idx_settings_audit_user ON settings_audit_log(changed_by)');
    await db.execute('CREATE INDEX idx_settings_audit_date ON settings_audit_log(created_at)');
    await db.execute('CREATE INDEX idx_settings_permissions_role ON settings_permissions(user_role)');
    await db.execute('CREATE INDEX idx_warehouse_settings_composite ON warehouse_settings(warehouse_id, setting_key)');
    await db.execute('''
    CREATE INDEX IF NOT EXISTS idx_settings_permissions_composite 
    ON settings_permissions(setting_key, user_role)
  ''');

  // أضف هذه الدالة في قسم _createIndexes:
      await db.execute('''
    CREATE INDEX IF NOT EXISTS idx_settings_user_view 
    ON app_settings(setting_key, category, display_order)
  ''');
  }

  Future<Map<String, dynamic>> getSettingWithDetails(String key) async {
    ...
    SELECT 
      ...
      sp.can_delete
    FROM app_settings s
    LEFT JOIN settings_permissions sp ON s.setting_key = sp.setting_key
    WHERE s.setting_key = ?
  ''', [key]);
    ...
    final options = await getSettingOptions(key);
    ...
    return {
      ...
      'visible_condition': setting['visible_condition'],
      'is_readonly': setting['is_readonly'],
      'requires_restart': setting['requires_restart'],
      'can_view': setting['can_view'] ?? 1,
      'can_edit': setting['can_edit'] ?? 0,
      'can_delete': setting['can_delete'] ?? 0,
    };
  }

  Future<Map<String, dynamic>> validateSetting(String key, dynamic value) async {
    final db = await database;
    ...
    if (result.isEmpty) {
      return {
        'valid': true,
        'message': 'الإعداد غير موجود، سيتم إنشاؤه',
        'can_create': true,
      };
    }
    ...
    final validationRegex = setting['validation_regex'] as String?;
    final minValue = setting['min_value'] as String?;
    final maxValue = setting['max_value'] as String?;
    final options = await getSettingOptions(key);
    ...
    if (settingType == 'int') {
      if (int.tryParse(stringValue) == null) {
        return {
          'valid': false,
          'message': 'القيمة يجب أن تكون رقماً صحيحاً',
      ...
      if (double.tryParse(stringValue) == null) {
        return {
          'valid': false,
          'message': 'القيمة يجب أن تكون رقماً عشرياً',
          'error_type': 'type_mismatch',
        };
      }
      ...
      if (stringValue != '0' && stringValue != '1' &&
        ...
        return {
          'valid': false,
          'message': 'القيمة يجب أن تكون true/false أو 0/1',
          'error_type': 'type_mismatch',
        };
      }
    }

    // التحقق من القيمة الدنيا والقصوى للأرقام
    if (settingType == 'int' || settingType == 'double') {
      final numValue = double.tryParse(stringValue);

      if (numValue != null && minValue != null) {
    ...
    if (options.isNotEmpty && !options.contains(stringValue)) {
      return {
        'valid': false,
        'message': 'القيمة يجب أن تكون واحدة من: ${options.join(", ")}',
  ...
  Future<Map<String, dynamic>> setSettingWithAudit(
      String key,
      dynamic value,
      int userId,
      {String? changeReason}
    ...
    try {
      final validation = await validateSetting(key, value);
      if (!validation['valid']) {
        return validation;
      }

      final oldValue = await getSetting(key);

      await db.transaction((txn) async {
        // الحصول على ID الإعداد
        final settingResult = await txn.query(
          'app_settings',
          columns: ['id'],
          where: 'setting_key = ?',
          whereArgs: [key],
        ...
        int settingId = 0;
        if (settingResult.isNotEmpty) {
          settingId = settingResult.first['id'] as int;
        }

        // تحديث أو إضافة الإعداد
        final result = await setSetting(key, value);
        ...
        await txn.insert('settings_audit_log', {
          'setting_id': settingId,
          'setting_key': key,
          'old_value': oldValue,
          'new_value': value.toString(),
          'changed_by': userId,
          'change_reason': changeReason ?? 'تغيير يدوي',
      ...
      return {
        'success': false,
        'error': 'فشل في حفظ الإعداد: ${e.toString()}',
        'audit_logged': false,
      };
    }
  }
  ...
  ''', whereArgs);
    ...
    return {
      ...
      'json_data': jsonEncode(settingsMap),
    };
  }

  Future<Map<String, dynamic>> importSettings(String jsonData, int userId) async {
    try {
      final Map<String, dynamic> settings = jsonDecode(jsonData);
      ...
      for (final entry in settings.entries) {
        final validation = await validateSetting(entry.key, entry.value);

        if (validation['valid']) {
          final result = await setSettingWithAudit(entry.key, entry.value, userId,
              changeReason: 'استيراد إعدادات'
          );
          ...
          errorCount++;
          errors.add('${entry.key}: ${validation['message']}');
        }
      }
      ...
      return {
        ...
        'errors': errors,
        'message': 'تم استيراد $successCount إعداد، فشل $errorCount إعداد',
      };
    } catch (e) {
      return {
        'success': false,
        'error': 'فشل في استيراد الإعدادات: ${e.toString()}',
      };
    }
  }

  Future<Map<String, dynamic>> getWarehouseSetting(
      int warehouseId,
      String settingKey,
      {String? defaultValue}
      ) async {
    ...
    final result = await db.query(
      'warehouse_settings',
      where: 'warehouse_id = ? AND setting_key = ?',
      whereArgs: [warehouseId, settingKey],
    );

    if (result.isNotEmpty) {
      return {
        'exists': true,
        'value': result.first['setting_value'],
        'type': result.first['setting_type'],
      };
    }

    // إذا لم يوجد إعداد للمخزن، جلب الإعداد العام
    final globalValue = await getSetting(settingKey, defaultValue: defaultValue);
  ...
  Future<String> getSetting(String key, {String? defaultValue}) async {
    ...
    // إذا لم توجد قيمة، حاول إرجاع القيمة الافتراضية من السجل
    if (result.isNotEmpty) {
      return result.first['default_value'] as String? ?? defaultValue ?? '';
    }
  ...
  Future<double> getDoubleSetting(String key, {double defaultValue = 0.0}) async {
    final value = await getSetting(key, defaultValue: defaultValue.toString());
  ...
  Future<List<Map<String, dynamic>>> getAllSettings() async {
    ...
    return await db.query(
      'app_settings',
      orderBy: 'category, group_name, display_order',
    );
  }
  ...
  Future<Map<String, dynamic>> setSetting(String key, dynamic value) async {
    ...
    try {
      ...
      return {'success': false, 'error': 'فشل في حفظ الإعداد: ${e.toString()}'};
    }
  }

  Future<Map<String, dynamic>> updateSetting(int id, Map<String, dynamic> data) async {
    final db = await database;
    ...
    try {
      ...
      return {'success': false, 'error': 'فشل في تحديث الإعداد: ${e.toString()}'};
    }
  }
  ...
  Future<Map<String, dynamic>> resetSettingToDefault(String key) async {
    ...
    try {
      ...
      if (setting.isNotEmpty) {
        final defaultValue = setting.first['default_value'] as String? ?? '';
        await db.update(
          'app_settings',
          {
      ...
      return {'success': true, 'message': 'تم استعادة الإعدادات الافتراضية'};
    } catch (e) {
      return {'success': false, 'error': 'فشل في استعادة الإعدادات: ${e.toString()}'};
    }
  }
  ...
  Future<Map<String, dynamic>> resetAllSettings() async {
    ...
    try {
      ...
      for (final setting in settings) {
        final defaultValue = setting['default_value'] as String? ?? '';
        await db.update(
          'app_settings',
          {
            'setting_value': defaultValue,
      ...
      return {'success': true, 'message': 'تم استعادة جميع الإعدادات الافتراضية'};
    } catch (e) {
      return {'success': false, 'error': 'فشل في استعادة الإعدادات: ${e.toString()}'};
    }
  }
  ...
  Future<List<String>> getSettingOptions(String key) async {
    final db = await database;
    final result = await db.query(
      'app_settings',
      columns: ['options'],
    ...
    if (result.isNotEmpty && result.first['options'] != null) {
      final optionsJson = result.first['options'] as String;
      try {
        final List<dynamic> parsed = jsonDecode(optionsJson);
...
// ========== دوال المستخدمين ==========
  ...
  Future<int> updateUser(int id, Map<String, dynamic> user) async {
    ...
    if (user.containsKey('password') && user['password'].isNotEmpty) {
      updateData['password'] = _hashPassword(user['password']);
    }
...
// ========== دوال النسخ الاحتياطي ==========
  Future<String> createBackup() async {
    ...
    final originalPath = join(databasesPath, 'inventory_system_v2.db');
    final backupPath = join(databasesPath, 'inventory_backup_${DateTime.now().millisecondsSinceEpoch}.db');
  ...
  Future<List<Map<String, dynamic>>> getAllAdvancedSettings() async {
    final db = await database;
    return await db.query('advanced_settings');
  }


  Future<List<Map<String, dynamic>>> getUserPermissions(int userId) async {
    final db = await database;
    return await db.query('user_permissions',
      where: 'user_id = ?',
      whereArgs: [userId],
    );
  }

  Future<int> updateUserPermission(int userId, String key, bool granted) async {
    final db = await database;

    // التحقق من وجود الصلاحية
    final existing = await db.query('user_permissions',
    ...
    if (existing.isNotEmpty) {
      // تحديث موجود
      return await db.update('user_permissions', {
        'granted': granted ? 1 : 0,
        'updated_at': DateTime.now().toIso8601String(),
      }, where: 'id = ?', whereArgs: [existing.first['id']]);
    } else {
      // إدخال جديد
      return await db.insert('user_permissions', {
        'user_id': userId,
        'permission_key': key,
        'granted': granted ? 1 : 0,
        'created_at': DateTime.now().toIso8601String(),
...
// الدوال الأخرى
  ...
  // المشكلة 2: getAllReturnPolicies()
  Future<List<Map<String, dynamic>>> getAllReturnPolicies() async {
    final db = await database;
    return await db.query('return_policies', orderBy: 'id DESC');
  }

  // المشكلة 3: updateSystemSetting()
  ...
  // المشكلة 7: deletePaymentTerm()
  Future<int> deletePaymentTerm(int id) async {
    final db = await database;
    return await db.delete(
      'payment_terms',
      where: 'id = ?',
  ...
  Future<int> createReturnPolicy(Map<String, dynamic> policy) async {
    final db = await database;
    return await db.insert('return_policies', policy);
  }

  Future<int> deleteReturnPolicy(int id) async {
    final db = await database;
    return await db.delete(
      'return_policies',
      where: 'id = ?',
...
// 3. دالة استعادة النسخ الاحتياطي
...
// 4. دالة اختبار اتصال قاعدة البيانات
  ...
  Future<Map<String, dynamic>> getSettings() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      'app_settings',
      where: 'category IN (?, ?, ?, ?, ?)',
      whereArgs: ['general', 'product', 'invoice', 'system', 'backup'],
    );

    Map<String, dynamic> settings = {};
    for (var row in maps) {
      String key = row['setting_key'];
      dynamic value = _convertSettingValue(
          row['setting_value'],
          row['setting_type']
      );
      settings[key] = value;
  ...
  Future<void> updateAdvancedSetting(String key, dynamic value) async {
    await updateSetting(key as int, value);
  }
  ...
  Future<Map<String, dynamic>> checkDatabaseIntegrity() async {
    try {
      ...
      // فحص السجلات اليتيمة
      final List<Map<String, dynamic>> orphanedRecords = [];

      // قائمة بجداول العلاقات الخارجية
      final foreignTables = [
        {'table': 'user_permissions', 'foreign_key': 'user_id', 'ref_table': 'users'},
        {'table': 'products', 'foreign_key': 'category_id', 'ref_table': 'categories'},
        {'table': 'products', 'foreign_key': 'supplier_id', 'ref_table': 'suppliers'},
        {'table': 'warehouse_stock', 'foreign_key': 'warehouse_id', 'ref_table': 'warehouses'},
        {'table': 'warehouse_stock', 'foreign_key': 'product_id', 'ref_table': 'products'},
      ];

      for (var ft in foreignTables) {
        final result = await db.rawQuery('''
          SELECT COUNT(*) as orphaned_count 
          FROM ${ft['table']} t 
          LEFT JOIN ${ft['ref_table']} r ON t.${ft['foreign_key']} = r.id 
          WHERE r.id IS NULL AND t.${ft['foreign_key']} IS NOT NULL
        ''');

        final count = result.first['orphaned_count'] as int;
        if (count > 0) {
          orphanedRecords.add({
            'table_name': ft['table']!,
            'foreign_key': ft['foreign_key']!,
            'ref_table': ft['ref_table']!,
      ...
      await db.insert('maintenance_logs', {
        ...
        'details': 'فحص سلامة قاعدة البيانات',
        'status': 'success',
      ...
      return {
        ...
        'data_integrity': orphanedRecords,
        'message': orphanedRecords.isEmpty
            ? '✅ قاعدة البيانات سليمة'
            : '⚠️ تم اكتشاف ${orphanedRecords.length} مشكلة في العلاقات',
      };
    } catch (e) {
      return {
        'success': false,
        'error': 'فشل فحص السلامة: $e',
      };
    }
  }
  ...
  Future<Map<String, dynamic>> getDatabaseStats() async {
    try {
      final db = await database;
      final databasesPath = await getDatabasesPath();
      final dbPath = join(databasesPath, 'inventory.db');
      ...
      final databaseSize = await dbFile.length();
      ...
      int totalRecords = 0;
      final List<Map<String, dynamic>> tableStats = [];
      ...
      return {
        'success': false,
        'error': 'فشل جلب الإحصائيات: $e',
      };
    }
  }

  dynamic _convertSettingValue(String? value, String type) {
  ...
  Future<Map<String, dynamic>> clearAllData() async {
    try {
      final db = await database;
      await db.transaction((txn) async {
        // قائمة الجداول التي يجب الحفاظ عليها (الإعدادات)
        final tables = [
          'products', 'customers', 'suppliers', 'sales', 'sales_items',
          'purchases', 'purchase_items', 'inventory_transactions',
          'cash_transactions', 'users', 'user_permissions'
        ];
      ...
      return {'success': false, 'error': 'فشل في مسح البيانات: $e'};
    }
  }


// دالة لإضافة/تحديث إعداد في قاعدة البيانات
  Future<void> setSystemSetting(String key, dynamic value) async {
    final db = await instance.database;

    // تحويل القيمة إلى نص لتخزينها
    ...
    if (existing.isNotEmpty) {
      ...
      await db.update(
        'settings',
        {'value': stringValue, 'updated_at': DateTime.now().toIso8601String()},
        where: 'key = ?',
        whereArgs: [key],
      );
    } else {
...
// دالة لإعادة تعيين جميع الإعدادات إلى القيم الافتراضية
  Future<void> resetSystemSettings() async {
    ...
    final defaultSettings = [
      {'key': 'company_name', 'value': 'شركة إدارة المخزون'},
      {'key': 'default_currency', 'value': 'ريال'},
      {'key': 'default_tax_rate', 'value': '15.0'},
      {'key': 'enable_notifications', 'value': 'true'},
      {'key': 'track_serial_numbers', 'value': 'false'},
      {'key': 'track_expiry_dates', 'value': 'false'},
      {'key': 'auto_print_invoice', 'value': 'false'},
      {'key': 'auto_purchase_orders', 'value': 'false'},
      {'key': 'email_reports', 'value': 'false'},
      {'key': 'two_factor_auth', 'value': 'false'},
      {'key': 'ip_restriction', 'value': 'false'},
      {'key': 'app_language', 'value': 'العربية'},
      {'key': 'app_theme', 'value': 'فاتح'},
      {'key': 'invoice_prefix', 'value': 'INV'},
      {'key': 'invoice_start_number', 'value': '1001'},
...
// دالة لجلب جميع إعدادات النظام

// دالة لإنشاء جدول الإعدادات (تضاف إلى دالة onCreate)
// أضف هذا في دالة onCreate بعد إنشاء الجداول الأخرى:
/*
await db.execute('''
...
// إصلاح دالة getAllSettings

  initializeDatabase() {}

  update(String s, Map<String, String> map, {required String where, required List<int> whereArgs}) {}

  query(String s, {required String where, required List<int> whereArgs}) {}

  Future<List<Map<String, dynamic>>> getUsersForChat() async {
    final db = await database;
    return await db.query(
      'users', // اسم جدول المستخدمين
      columns: ['id', 'name', 'email'], // الأعمدة المطلوبة
      where: 'is_active = ?', // شروط إذا لزم الأمر
      whereArgs: [1],
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getLatestSales(int limit) async {
    final db = await database;
    return await db.query(
      'sales',
      orderBy: 'date DESC',
  ...
  ''', [_getStartDate(period)]);
    ...
    if (result.isNotEmpty) {
      ...
      return {
        ...
        'total_profit': row['total_profit'] ?? 0.0,
...
// في database_helper.dart
  ...
  Future<List<Map<String, dynamic>>> getProductsForChat({int limit = 10}) async {
  final db = await database;
  return await db.rawQuery('''
      SELECT
        id, name, barcode,
...
Future<Map<String, dynamic>?> getProductForChat(String identifier) async {
  final db = await database;
  final result = await db.rawQuery('''
      SELECT 
        p.*,
      ...
      LEFT JOIN categories c ON p.category_id = c.id
      LEFT JOIN suppliers s ON p.supplier_id = s.id
      WHERE (p.barcode = ? OR p.name LIKE ? OR p.code = ?) 
        AND p.is_active = 1
      LIMIT 1
  ...
  if (result.isNotEmpty) {
    ...
    final costPrice = product['cost_price'] as double? ?? 0.0;
    final sellPrice = product['sell_price'] as double? ?? 0.0;
...
Future<List<Map<String, dynamic>>> getLowStockProductsForChat() async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        p.id,
        ...
        p.current_quantity,
        p.min_stock_level,
        (p.current_quantity * 100.0 / p.min_stock_level) as stock_percentage
      FROM products p
      WHERE p.is_active = 1 
        AND p.min_stock_level > 0 
        AND p.current_quantity <= p.min_stock_level
      ORDER BY p.current_quantity ASC
      LIMIT 20
    ''');
}

Future<List<Map<String, dynamic>>> getOutOfStockProductsForChat() async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        p.id,
...
Future<List<Map<String, dynamic>>> getSuppliersForChat({int limit = 10}) async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        s.id,
...
Future<Map<String, dynamic>?> getSupplierForChat(String name) async {
  final db = await database;
  final result = await db.rawQuery('''
      SELECT 
        s.*,
        COUNT(p.id) as product_count,
        SUM(p.current_quantity * p.cost_price) as total_investment
      FROM suppliers s
      LEFT JOIN products p ON s.id = p.supplier_id
...
Future<List<Map<String, dynamic>>> getCustomersForChat({int limit = 10}) async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        c.id,
...
Future<Map<String, dynamic>?> getCustomerForChat(String name) async {
  final db = await database;
  final result = await db.rawQuery('''
      SELECT 
        c.*,
  ...
  if (result.isNotEmpty) {
    ...
    return {
      ...customer,
      'invoices': invoices,
      'total_remaining': (customer['total_spent'] as double? ?? 0) -
          (customer['total_paid'] as double? ?? 0),
    };
  }
  return null;
}

Future<List<Map<String, dynamic>>> getWarehousesForChat() async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        w.*,
...
Future<Map<String, dynamic>> getWarehouseStockForChat(int warehouseId) async {
  final db = await database;
  final result = await db.rawQuery('''
      SELECT 
        w.name as warehouse_name,
  ...
  final warehouseData = result.first;
...
Future<Map<String, dynamic>> getFinancialSummaryForChat(String period) async {
  final db = await database;
  ...
  final salesResult = await db.rawQuery('''
      ...
      FROM sale_invoices
      WHERE status = 'approved' AND $dateFilter
    ''');
  ...
  // الأرباح من بنود المبيعات
  final profitResult = await db.rawQuery('''
      SELECT 
        SUM(si.quantity * (si.unit_price - si.cost_price)) as total_profit
      FROM sale_items si
      JOIN sale_invoices s ON si.sale_invoice_id = s.id
      WHERE s.status = 'approved' AND $dateFilter
    ''');
  ...
  final debtsResult = await db.rawQuery('''
      ...
      FROM (
        ...
        UNION ALL
        SELECT -balance FROM suppliers WHERE balance < 0
      )
    ''');

  final totalSales = salesResult.first['total_sales'] as double? ?? 0;
  final totalPurchases = purchasesResult.first['total_purchases'] as double? ?? 0;
  final totalProfit = profitResult.first['total_profit'] as double? ?? 0;
  final totalDebts = debtsResult.first['total_debts'] as double? ?? 0;
  ...
  return {
    ...
    'net_income': totalSales - totalPurchases,
  };
}

Future<List<Map<String, dynamic>>> getRecentOperationsForChat(int limit) async {
  final db = await database;
  ...
  return await db.rawQuery('''
      ...
      SELECT 
        'receipt' as type,
        voucher_number as number,
        amount,
        payment_date as date,
        'سند قبض' as description
...
Future<Map<String, dynamic>?> getUserForChat(String username) async {
  final db = await database;
  final result = await db.rawQuery('''
      SELECT 
        u.*,
        COUNT(DISTINCT si.id) as sales_count,
        COUNT(DISTINCT pi.id) as purchases_count
      FROM users u
      LEFT JOIN sale_invoices si ON u.id = si.created_by
      LEFT JOIN purchase_invoices pi ON u.id = pi.created_by
...
Future<List<Map<String, dynamic>>> getRecentLoginsForChat(int limit) async {
  final db = await database;
  return await db.rawQuery('''
      SELECT 
        username,
...
// دوال متقدمة للدردشة الذكية
  Future<Map<String, dynamic>> getAdvancedSystemInfo() async {
    final db = await database;

    // معلومات النظام الشاملة
    final Map<String, dynamic> info = {};

    // 1. معلومات المستخدمين
    final usersInfo = await db.rawQuery('''
    SELECT 
    ...
    FROM users
  ''');
    info['users'] = usersInfo.first;

    // 2. معلومات المنتجات
    final productsInfo = await db.rawQuery('''
    SELECT 
      COUNT(*) as total_products,
      SUM(current_quantity) as total_stock,
      SUM(current_quantity * sell_price) as total_stock_value,
      AVG(sell_price - purchase_price) as avg_profit_margin,
      COUNT(CASE WHEN current_quantity = 0 THEN 1 END) as out_of_stock,
      COUNT(CASE WHEN current_quantity > 0 AND current_quantity <= min_stock_level THEN 1 END) as low_stock
    FROM products
    WHERE is_active = 1
  ''');
    info['products'] = productsInfo.first;

    // 3. معلومات المخازن
    final warehousesInfo = await db.rawQuery('''
    SELECT 
      COUNT(*) as total_warehouses,
      SUM(CASE WHEN is_active = 1 THEN 1 ELSE 0 END) as active_warehouses,
      (SELECT COUNT(DISTINCT product_id) FROM warehouse_stock) as unique_products_in_warehouses,
      (SELECT SUM(quantity) FROM warehouse_stock) as total_items_in_warehouses
    FROM warehouses
  ''');
    info['warehouses'] = warehousesInfo.first;

    // 4. معلومات المالية
    final financialInfo = await db.rawQuery('''
    SELECT 
      (SELECT COALESCE(SUM(total_amount), 0) FROM sale_invoices WHERE status = 'approved') as total_sales,
      (SELECT COALESCE(SUM(total_amount), 0) FROM purchase_invoices WHERE status = 'approved') as total_purchases,
      (SELECT COALESCE(SUM(balance), 0) FROM customers WHERE balance > 0) as customer_debts,
      (SELECT COALESCE(SUM(balance), 0) FROM suppliers WHERE balance < 0) as supplier_debts,
      (SELECT COALESCE(balance_after, 0) FROM cash_ledger ORDER BY id DESC LIMIT 1) as current_cash_balance
  ''');
    info['financial'] = financialInfo.first;

    // 5. معلومات النشاط اليوم
    final today = DateFormat('yyyy-MM-dd').format(DateTime.now());
    final todayInfo = await db.rawQuery('''
    SELECT 
      (SELECT COUNT(*) FROM sale_invoices WHERE date(invoice_date) = ? AND status = 'approved') as today_sales_count,
      (SELECT COALESCE(SUM(total_amount), 0) FROM sale_invoices WHERE date(invoice_date) = ? AND status = 'approved') as today_sales_amount,
      (SELECT COUNT(*) FROM purchase_invoices WHERE date(invoice_date) = ? AND status = 'approved') as today_purchases_count,
      (SELECT COALESCE(SUM(total_amount), 0) FROM purchase_invoices WHERE date(invoice_date) = ? AND status = 'approved') as today_purchases_amount,
      (SELECT COUNT(*) FROM audit_log WHERE date(created_at) = ?) as today_activities
  ''', [today, today, today, today, today]);
    info['today'] = todayInfo.first;
    ...
    info['settings_count'] = settingsCount.first['count'];
  ...
  Future<Map<String, dynamic>> getUserDetailedInfo(int userId) async {
    final db = await database;

    final userInfo = await db.rawQuery('''
    SELECT 
      u.*,
      COUNT(DISTINCT si.id) as sales_invoices_count,
      COUNT(DISTINCT pi.id) as purchase_invoices_count,
  ...
  ''', [userId]);
    ...
    // آخر الأنشطة
    final recentActivities = await db.rawQuery('''
  ...
  ''', [userId]);
    ...
    return {
      ...
      'granted_permissions': permissions.where((p) => p['granted'] == 1).length,
      'denied_permissions': permissions.where((p) => p['granted'] == 0).length,
    };
  }

  Future<Map<String, dynamic>> getWarehouseDetailedInfo(String warehouseName) async {
    final db = await database;

    final warehouse = await db.rawQuery('''
  ...
  ''', ['%$warehouseName%', '%$warehouseName%']);
    ...
    final warehouseId = warehouse.first['id'];

    // المنتجات في المخزن مع تفاصيل
    final products = await db.rawQuery('''
    SELECT 
      p.*,
      ...
      s.name as supplier_name,
      (p.sell_price - p.cost_price) as profit_per_unit,
      (ws.quantity * (p.sell_price - p.cost_price)) as total_profit_in_warehouse
    FROM warehouse_stock ws
    JOIN products p ON ws.product_id = p.id
    LEFT JOIN categories c ON p.category_id = c.id
    LEFT JOIN suppliers s ON p.supplier_id = s.id
  ...
  ''', [warehouseId]);
    ...
    SELECT 
      ...
      SUM(ws.quantity) as total_quantity,
      SUM(ws.quantity * p.sell_price) as total_value,
      SUM(ws.quantity * p.cost_price) as total_cost,
      SUM(ws.quantity * (p.sell_price - p.cost_price)) as total_profit,
      AVG(p.sell_price) as avg_sell_price,
      AVG(p.cost_price) as avg_cost_price,
      AVG(p.sell_price - p.cost_price) as avg_profit_per_unit,
      SUM(CASE WHEN ws.quantity = 0 THEN 1 ELSE 0 END) as out_of_stock_count,
      SUM(CASE WHEN ws.quantity > 0 AND ws.quantity <= p.min_stock_level THEN 1 ELSE 0 END) as low_stock_count,
      SUM(CASE WHEN ws.quantity > p.min_stock_level THEN 1 ELSE 0 END) as good_stock_count
    FROM warehouse_stock ws
    JOIN products p ON ws.product_id = p.id
    WHERE ws.warehouse_id = ?
      AND p.is_active = 1
  ''', [warehouseId]);
    ...
    LEFT JOIN users u ON pm.reference_id = u.id
    WHERE pm.notes LIKE ?
  ...
  ''', ['%${warehouse.first['name']}%']);
    ...
    return {
      ...
      'products': products,
      'stats': stats.isNotEmpty ? stats.first : {},
      'movements': movements,
      'products_count': products.length,
    };
  }

  Future<List<Map<String, dynamic>>> getSystemAlertsSummary() async {
    final db = await database;
  ...
  Future<Map<String, dynamic>> _getTodayLoginStats(Database db) async {
    final today = DateFormat('yyyy-MM-dd').format(DateTime.now());
    ...
    SELECT 
      ...
      u.name,
      u.role,
      MAX(al.created_at) as last_login_time,
  ...
  ''');
    ...
    final todayStats = summary.isNotEmpty ? summary.first : {};
    final totalActiveUsers = totalUsers.first['total'] as int? ?? 0;
    final todayUniqueUsers = todayStats['unique_users_today'] as int? ?? 0;

    String response = '''
🔐 **إحصائيات تسجيلات الدخول اليوم**

📊 **المستخدمون النشطون اليوم:**
• عدد المستخدمين الفريدين: $todayUniqueUsers من أصل $totalActiveUsers
• إجمالي عمليات الدخول: ${todayStats['total_logins_today'] ?? 0}

👥 **المستخدمون النشطون:**
${result.take(10).map((user) {
      final username = user['username'] ?? 'غير معروف';
      final name = user['name'] ?? 'بدون اسم';
      final role = user['role'] ?? 'غير محدد';
      final loginTime = user['last_login_time'] != null
          ? DateFormat('hh:mm a').format(DateTime.parse(user['last_login_time'] as String))
          : 'غير مسجل';

      return '• **$name** ($username) - ${_translateRole(role as String)} - آخر دخول: $loginTime';
    }).join('\n')}

${result.length > 10 ? '\n... و ${result.length - 10} مستخدم آخر' : ''}

📈 **معدل النشاط اليوم:** ${((todayUniqueUsers / totalActiveUsers) * 100).toStringAsFixed(1)}%
  ''';
  ...
  Future<Map<String, dynamic>> _getRecentLoginStats(Database db) async {
    final lastWeek = DateFormat('yyyy-MM-dd').format(
    ...
    WHERE u.is_active = 1
    GROUP BY u.id
    HAVING last_login IS NOT NULL
    ORDER BY u.last_login DESC
    LIMIT 15
  ''', [lastWeek]);

    final neverLoggedIn = await db.rawQuery('''
  ...
  ''', [lastWeek]);
    ...
    final neverCount = neverLoggedIn.first['count'] as int? ?? 0;

    String response = '''
📅 **آخر عمليات تسجيل الدخول (آخر 7 أيام)**

👤 **آخر 15 مستخدم قاموا بتسجيل الدخول:**

${result.take(10).map((user) {
      final index = result.indexOf(user) + 1;
      final username = user['username'] ?? 'غير معروف';
      final name = user['name'] ?? 'بدون اسم';
      final role = _translateRole(user['role'] as String? ?? '');
      final lastLogin = user['last_login'] != null
          ? DateFormat('yyyy-MM-dd HH:mm').format(DateTime.parse(user['last_login'] as String))
          : 'غير مسجل';
      final loginCount = user['login_count'] ?? 0;


    }).join('\n\n')}

⚠️ **المستخدمون الذين لم يسجلوا دخولاً أبداً:** $neverCount مستخدم

📊 **تحليل الأسبوع:**
${weeklyStats.take(5).map((day) {
      final date = DateTime.parse(day['login_date'] as String);
      final dateStr = DateFormat('yyyy-MM-dd').format(date);
      final unique = day['unique_users'] ?? 0;
      final total = day['total_logins'] ?? 0;

      return '• $dateStr: $unique مستخدم ($total عملية دخول)';
    }).join('\n')}
  ''';
...
// دالة مساعدة لترجمة الأدوار
  ...
  String _generateUserAnswer(Map<String, dynamic> data) {
    ...
    String answer = 'المستخدمون:\n';
    answer += '• إجمالي المستخدمين: $totalUsers\n';
    answer += '• عدد المديرين: $adminCount\n';
    answer += '• عدد المدراء: $managerCount\n';
    answer += '• عدد أمناء الصندوق: $cashierCount\n';
    answer += '• عدد مسؤولي المخازن: $warehouseCount\n';
    answer += '• عدد المشاهدين: $viewerCount\n';

    return answer;
  }

  String _generateWarehouseAnswer(Map<String, dynamic> data) {
    List<dynamic> warehouses = data['warehouses'] ?? [];
    int totalWarehouses = warehouses.length;

    double totalStockValue = 0;
    warehouses.forEach((warehouse) {
      totalStockValue += warehouse['total_value'] ?? 0;
    });

    String answer = 'المخازن:\n';
    answer += '• عدد المخازن: $totalWarehouses\n';
    answer += '• إجمالي قيمة المخزون في جميع المخازن: ${totalStockValue.toStringAsFixed(2)} ريال\n\n';

    answer += 'تفاصيل المخازن:\n';
    for (var warehouse in warehouses) {
      answer += '• ${warehouse['name']}: ${warehouse['total_quantity'] ?? 0} وحدة، القيمة: ${warehouse['total_value']?.toStringAsFixed(2) ?? '0'} ريال\n';
    }

    return answer;
  }

  String _generateProfitAnswer(Map<String, dynamic> data) {
    double totalProfit = data['total_profit'] ?? 0;
    double totalRevenue = data['total_revenue'] ?? 0;
    double totalCost = data['total_cost'] ?? 0;
    double profitMargin = totalRevenue != 0 ? (totalProfit / totalRevenue) * 100 : 0;

    String answer = 'الأرباح:\n';
    answer += '• إجمالي الإيرادات: ${totalRevenue.toStringAsFixed(2)} ريال\n';
    answer += '• إجمالي التكاليف: ${totalCost.toStringAsFixed(2)} ريال\n';
    answer += '• إجمالي الأرباح: ${totalProfit.toStringAsFixed(2)} ريال\n';
    answer += '• هامش الربح: ${profitMargin.toStringAsFixed(2)}%\n';

    return answer;
  }

  String _generateRecentAnswer(Map<String, dynamic> data) {
    List<dynamic> recentOperations = data['recent_operations'] ?? [];

    String answer = 'العمليات الأخيرة:\n';

    for (int i = 0; i < (recentOperations.length < 10 ? recentOperations.length : 10); i++) {
      var operation = recentOperations[i];
      answer += '${i+1}. ${operation['description']} - ${operation['date']}\n';
    }

    return answer;
  }

  String _generateLowStockAnswer(Map<String, dynamic> data) {
    List<dynamic> lowStockProducts = data['low_stock_products'] ?? [];

    String answer = 'المنتجات المنخفضة بالمخزون:\n';

    if (lowStockProducts.isEmpty) {
      answer += 'لا توجد منتجات منخفضة المخزون حالياً.';
    } else {
      for (var product in lowStockProducts) {
        answer += '• ${product['name']}: المخزون الحالي ${product['current_quantity'] ?? 0}، الحد الأدنى ${product['min_stock_level'] ?? 0}\n';
      }
    }
  ...
  String _generateAlertAnswer(Map<String, dynamic> data) {
    ...
    String answer = 'التنبيهات:\n';
    answer += '• إجمالي التنبيهات: ${alerts.length}\n';
    answer += '• التنبيهات غير المقروءة: $unreadAlerts\n\n';

    // عرض التنبيهات الحرجة أولاً
    answer += 'التنبيهات الحرجة:\n';
    for (var alert in alerts) {
      if (alert['priority'] == 'critical' && alert['is_read'] == 0) {
        answer += '⚠️ ${alert['title']}: ${alert['message']}\n';
      }
    }
